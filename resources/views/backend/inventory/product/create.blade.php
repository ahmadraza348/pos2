...
{{-- Replace the entire "Pricing Setup" card with this --}}
<div class="col-12">
    <div class="card border-primary">
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-6 mb-3">
                    <label class="form-label" for="minimum_stock">Minimum Stock Alert</label>
                ...
                <div class="col-6 mb-3">
                    ...
                    <small class="text-muted">Optional — fills selling price automatically.</small>
                </div>

                <div class="col-12 mb-0">
                    <label class="form-label fw-bold" for="selling_price">Selling Price *</label>
                    <div class="input-group">
                        <span class="input-group-text">Rs.</span>
                        <input type="number" step="0.01" name="selling_price" id="selling_price" required min="0"
                            class="form-control fw-bold @error('selling_price') is-invalid @enderror"
                            value="{{ old('selling_price', '') }}"
                            placeholder="0.00">
                    </div>
                    @error('selling_price') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    <small class="text-muted">Auto-fills from purchase price + margin, or type it directly.</small>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function autoFillSelling() {
        const cost   = parseFloat(costInput.value)   || 0;
        const margin = parseFloat(marginInput.value) || 0;

        if (cost > 0 && margin > 0) {
            sellingInput.value = (cost * (1 + margin / 100)).toFixed(2);
        }
    }

    // Auto-fill selling price when cost OR margin changes,
    // but only if the cashier hasn't manually edited selling_price.
    let manualSellingEdit = false;

    sellingInput.addEventListener('input', function () {
        manualSellingEdit = true;
    });

    costInput.addEventListener('input', function () {
        if (!manualSellingEdit) autoFillSelling();
    });
